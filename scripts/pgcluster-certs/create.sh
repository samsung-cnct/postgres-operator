...
${DIR?}/ssl-creator.sh "testuser_crunchydata.com" "${CONTAINER_NAME?}" "${DIR}"
if [[ $? -ne 0 ]]
then
...
kubectl delete configmap pgo-custom-ssl-config -n ${PGO_NAMESPACE}

kubectl create --namespace=${PGO_NAMESPACE?} configmap pgo-custom-ssl-config \
    --from-file=${DIR?}/configs/ca.crt \
    --from-file=${DIR?}/configs/ca.crl \
    --from-file=${DIR?}/certs/client.crt \
    --from-file=${DIR?}/certs/client.key \
...
sslmode=verify-full&\
sslrootcert=/configs/ca.crt&\
sslcrl=/configs/ca.crl&\
sslcert=/configs/client.crt&\
sslkey=/configs/client.key\""
echo ""
