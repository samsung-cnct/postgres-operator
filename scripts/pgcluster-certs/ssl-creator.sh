...
if [[ ! -x "$(command -v $GOBIN/certstrap)" ]]
then
...
$GOBIN/certstrap --depot-path ${OUTPUT_DIR?}/out \
...
$GOBIN/certstrap --depot-path ${OUTPUT_DIR?}/out request-cert --passphrase '' --common-name ${SERVER?} --domain ${SERVER?},"${SERVER?}-replica","${SERVER?}-pgpool"
$GOBIN/certstrap --depot-path ${OUTPUT_DIR?}/out sign ${SERVER?} --passphrase '' --CA RootCA --years 1

$GOBIN/certstrap --depot-path ${OUTPUT_DIR?}/out request-cert --passphrase '' --common-name ${USERNAME?}
$GOBIN/certstrap --depot-path ${OUTPUT_DIR?}/out sign ${USERNAME?} --passphrase '' --CA RootCA --years 1
